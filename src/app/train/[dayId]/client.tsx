...
export function TrainingSession() {
  ...
  async function loadContent() {
    ...
    try {
      ...
      const resp = await fetch(`/content/week-${week}-day-${day}.json`);
      if (resp.ok) {
  ...
  async function handleCompleteStep() {
    ...
    if (allComplete && currentStep === 'review') {
      ...
      await saveSession(today, { date: today, week, day, stepsCompleted: allSteps.map(() => true), struggleNotes: stepStates.review?.struggleNotes || '', minutesSpent: mins });
  ...
  if (error) {
    return (
      <div className="space-y-4 py-12 text-center">
        ...
        <button onClick={loadContent} className="rounded-lg bg-primary-600 px-4 py-2 text-white hover:bg-primary-700">重试</button>
        <button onClick={() => router.push('/train')} className="ml-2 rounded-lg border border-slate-300 px-4 py-2 dark:border-slate-600">返回列表</button>
      </div>
    );
  }
  ...
  if (stepStates.review?.completed) {
    return (
      <div className="space-y-6 py-12 text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400">完成了 Week {week} Day {day} 的训练，用时 {totalMinutes || Math.round(timer / 60)} 分钟。</p>
        <div className="flex justify-center gap-3">
          <button onClick={() => router.push('/train')} className="rounded-lg bg-primary-600 px-4 py-2 text-white hover:bg-primary-700">继续训练</button>
          <button onClick={() => router.push('/')} className="rounded-lg border border-slate-300 px-4 py-2 dark:border-slate-600">返回首页</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold">Week {week} Day {day}: {content?.title}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">{STEP_LABELS[currentStep]} · {Math.floor(timer / 60)}:{(timer % 60).toString().padStart(2, '0')}</p>
        </div>
        {!timerActive && timer === 0 && (
          <button onClick={() => setTimerActive(true)} className="rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-700">开始计时</button>
        )}
      </div>
      <StepNavigation steps={steps} currentStep={currentStep} stepStates={stepStates} onStepClick={(s) => setStep(s)} labels={STEP_LABELS} />
      <div className="min-h-[400px] rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
        {currentStep === 'sentence_frames' && content && (
          <SentenceFrameDrill frames={content.sentenceFrames} onComplete={() => handleCompleteStep()} completed={stepStates.sentence_frames?.completed || false} />
        )}
        {currentStep === 'collocations' && content && (
          <CollocationBurst collocations={content.collocations} onComplete={() => handleCompleteStep()} completed={stepStates.collocations?.completed || false} onMarkItems={(items) => updateStepState('collocations', { markedItems: items })} />
        )}
        {currentStep === 'shadowing' && content && (
          <ShadowingPractice youtubeSearch={content.shadowingTopic} onComplete={() => handleCompleteStep()} completed={stepStates.shadowing?.completed || false} />
        )}
        {currentStep === 'impromptu' && content && (
          <ImpromptuSpeaking topic={content.impromptuTopic} cueWords={content.impromptuCueWords} onComplete={() => handleCompleteStep()} completed={stepStates.impromptu?.completed || false} onAudioSaved={(key) => updateStepState('impromptu', { audioBlobKey: key })} />
        )}
        {currentStep === 'review' && (
          <ReviewReflection onComplete={() => handleCompleteStep()} completed={stepStates.review?.completed || false} onSaveNotes={(notes) => updateStepState('review', { struggleNotes: notes })} />
        )}
      </div>
    </div>
  );
